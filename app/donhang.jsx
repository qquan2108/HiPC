...
const OrderCard = React.memo(
  ({ order, tab, onCancel, onStatusChange, onReview }) => {
    ...
    const combosInProductsCount =
      order.products
        ?.filter((p) => p.comboId)
        .reduce((sum, c) => sum + (c.quantity || 0), 0) || 0;

    const combosArrayCount =
      order.combos?.reduce((sum, c) => sum + (c.quantity || 0), 0) || 0;

    const itemCount = productCount + combosInProductsCount + combosArrayCount;
      ...
      >
        ...
        >
                ...
                {tab && (
                  <>
                    <MaterialCommunityIcons
                      ...
                      color="#FFFFFF"
                    />
                    <Text style={[styles.statusText, { color: "#221f1fff" }]}>
                      {tab.label}
                    </Text>
                  </>
                )}
          ...
          <View style={styles.productsContainer}>
            {/* Individual Products */}
            {order.products &&
              order.products.filter((p) => p.productId).length > 0 &&
              order.products
                .filter((prod) => prod.productId)
                .map((prod) => (
                  <View key={prod._id} style={styles.productRow}>
                    {/* 🆕 Combo xuất hiện trong mảng order.products (có comboId) */}
                    {order.products &&
                      order.products.filter((it) => it.comboId).length > 0 &&
                      order.products
                        .filter((it) => it.comboId)
                        .map((item, idx) => {
                          const comboName =
                            item.comboDetails?.name ||
                            item.comboId?.name ||
                            "Combo";
                          const comboProducts =
                            item.comboDetails?.products ||
                            item.comboId?.productIds ||
                            [];
                          const comboImage =
                            item.comboDetails?.image ||
                            item.comboId?.image ||
                            comboProducts[0]?.image ||
                            comboProducts[0]?.productId?.image;

                          // Ưu tiên giá từ item.price -> comboDetails.price -> comboId.price -> 0
                          const comboPrice = Number(
                            item.price ??
                              item.comboDetails?.price ??
                              item.comboId?.price ??
                              0
                          );

                          // Mô tả sản phẩm trong combo + biến thể nếu có
                          const comboDesc = comboProducts
                            .map((p, i) => {
                              const prodObj = p.productId || p;
                              const vLabel =
                                p.variant?.label ||
                                p.selectedVariant?.label ||
                                "";
                              return `${i ? " • " : ""}${prodObj?.name || ""}${
                                vLabel ? ` (${vLabel})` : ""
                              }`;
                            })
                            .join("");
                          ...
                          return (
                            <View
                              key={`combo-in-products-${item._id || idx}`}
                            ...
                            >
                              ...
                              <View style={styles.productInfo}>
                                <Text
                                  numberOfLines={2}
                                  style={[styles.productName, styles.comboName]}
                                >
                                  🎁 {comboName}
                                </Text>
                                <Text style={styles.comboDescription}>
                                  {comboDesc ||
                                    `${comboProducts.length} sản phẩm`}
                                </Text>
                    ...
                    <View style={styles.productInfo}>
                      ...
                      <View style={styles.productPriceContainer}>
                        {/* 🆕 Giá theo variant: base + price/priceDiff */}
                        <Text style={styles.productPrice}>
                          {formatCurrency(
                            Number(prod.productId?.price ?? 0) +
                              Number(
                                prod.variant?.price ??
                                  prod.variant?.priceDiff ??
                                  0
                              )
                          )}
                        </Text>
            ...
            {order.combos &&
              ...
              order.combos.map((combo, idx) => {
                const comboProducts =
                  combo.comboDetails?.products ?? // BE mới có thể đã build sẵn
                  combo.products ?? // nếu bạn lưu sp đã chọn trong combos.products
                  combo.comboId?.productIds ??
                  []; // fallback: danh sách gốc của combo

                const comboImage =
                  combo.comboDetails?.image ||
                  combo.comboId?.image ||
                  comboProducts[0]?.image ||
                  comboProducts[0]?.productId?.image ||
                  null;
                ...
                const comboDesc = comboProducts
                  .map((p, i) => {
                    ...
                    const vLabel = p.variant?.label || p.variant?.name || "";
                    return `${i ? " • " : ""}${prod?.name || ""}${
                      vLabel ? ` (${vLabel})` : ""
                    }`;
                  })
                ...
                return (
                  <View key={combo._id || idx} style={styles.productRow}>
                    ...
                    <View style={styles.productInfo}>
                      ...
                      >
                        🎁{" "}
                        {combo.comboDetails?.name ||
                          combo.comboId?.name ||
                          "Premium PC Combo"}
                      </Text>

                      <Text numberOfLines={2} style={styles.comboDescription}>
                        {comboDesc || `${comboProducts.length} sản phẩm`}
                      </Text>
                    ...
                    <View style={styles.quantityContainer}>
                      <LinearGradient
                        ...
                        style={styles.quantityBadge}
                      >
                        <Text style={[styles.productQty, { color: "#92400E" }]}>
                          ×{combo.quantity || 1}
                        </Text>
                      </LinearGradient>
                    </View>
...
export default function TrackOrderScreen() {
  ...
  const renderScene = ({ route }) => {
    const filtered = orders
      .filter(
        (o) =>
          !(o.status === "pending" && !o.products?.length && !o.combos?.length)
      )
          ...
          renderLabel={({ route, focused }) => (
            ...
            >
              <LinearGradient
                colors={
                  focused
                    ? route.gradient || ["#6366F1", "#8B5CF6"]
                    : ["transparent", "transparent"]
                }
                style={[
                  styles.tabIconContainer,
                  !focused && { backgroundColor: "#F1F5F9" },
                ]}
              >
                <MaterialCommunityIcons
                  name={route.icon}
