...
const getVariantId = (v) =>
  v?._id ||
  v?.id ||
  (v ? `${v.key || ""}-${v.label || v.name || ""}` : undefined);

const withSafeId = (opt, idx = 0, groupKey = "") => {
  if (!opt) return opt;
  const safeId =
    opt._id ||
    opt.id ||
    `${groupKey || opt.key || "opt"}-${(opt.label || opt.name || "option")
      .toString()
      .trim()
      .toLowerCase()
      .replace(/\s+/g, "-")}-${idx}`;
  return { ...opt, _id: safeId };
};
...
const extractVariantOptions = (product) => {
  ...
  if (Array.isArray(v[0]?.options)) {
    const groupKey = v[0]?.key || "option";
    return v[0].options.map((o, idx) => withSafeId(o, idx, groupKey));
  }

  return v.map((o, idx) => withSafeId(o, idx));
};

// Ưu tiên option.price; nếu không có thì base + priceDiff
...
export default function PCChatBoxAI() {
  ...
  const initializeConversation = () => {
    setConversationHistory([
      { role: "user", parts: [{ text: SYSTEM_PROMPT }] },
      {
        ...
        parts: [
          {
            text: "Tôi hiểu. Tôi sẽ hoạt động như HiPC AI Assistant - chuyên gia tư vấn PC tại Việt Nam với phong cách thân thiện và chuyên nghiệp. Tôi sẵn sàng hỗ trợ bạn!",
          },
        ],
      },
    ]);
  ...
  const detectCategory = (input) => {
    ...
    if (lowerInput.includes("build") || lowerInput.includes("cấu hình"))
      return "build";
    if (lowerInput.includes("cpu") || lowerInput.includes("processor"))
      return "cpu";
    if (lowerInput.includes("gpu") || lowerInput.includes("vga")) return "gpu";
    if (lowerInput.includes("ram") || lowerInput.includes("bộ nhớ"))
      return "ram";
    if (lowerInput.includes("case") || lowerInput.includes("vỏ máy"))
      return "case";
    if (lowerInput.includes("lỗi") || lowerInput.includes("trouble"))
  ...
  const extractProductsFromResponse = (response) => {
    ...
    const productRegex =
      /^\s*[*-]\s*(.+?),\s*giá\s*([\d\.,]+)\s*(?:VND|vnđ|đ)?\s*(?:\(ID:\s*([^)]+)\))?/i;
    const products = [];
    const introLines = [];
    lines.forEach((line) => {
  ...
  const fetchProductDetails = async (name) => {
    try {
      ...
      const { data } = await axiosInstance.get(
        "/product/filter-keyword?" + params.toString()
      );
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const { products: extractedProducts, intro } =
        extractProductsFromResponse(aiResponse);
      ...
      if (extractedProducts.length > 0) {
        ...
        const products = detailed.map((d, idx) =>
          d
            ? d
            : {
                _id: extractedProducts[idx].sku || idx,
                name: extractedProducts[idx].name,
                price: extractedProducts[idx].price,
              }
        );
  ...
  const parseBasePrice = (priceLike) => {
    ...
    if (typeof priceLike === "string")
      return Number(priceLike.replace(/[^\d]/g, "")) || 0;
    return 0;
  };
  ...
  // Validate số lượng với server (giống ForYouGrid)
  const apiValidateQuantity = async (qty) => {
    ...
    try {
      ...
      if (allowed === false) {
        ...
        setVariantQuantity((q) => Math.min(q, limit));
  ...
  const pickDefaultVariant = (product) => {
    const opts = extractVariantOptions(product);
    if (opts.length > 0) {
      const firstInStock =
        opts.find((o) => Number(o?.stock ?? 0) > 0) || opts[0];
      setSelectedVariant(firstInStock);
      const startQty = Number(firstInStock?.stock ?? 0) > 0 ? 1 : 0;
      setVariantQuantity(startQty);
      if (startQty > 0) {
        // gọi validate số lượng ban đầu
        apiValidateQuantity(startQty);
      }
    } else {
      const defaultVariant = {
        _id: `${getProductId(product)}-default`,
        label: "Mặc định",
        stock: Number(product?.stock ?? 0),
        price: Number(product?.price ?? 0),
      };
      setSelectedVariant(defaultVariant);
      const startQty = defaultVariant.stock > 0 ? 1 : 0;
      setVariantQuantity(startQty);
      if (startQty > 0) {
        apiValidateQuantity(startQty);
      }
  ...
  const handleVariantSelect = (variant) => {
    const v = withSafeId(variant); // đảm bảo có _id
    setSelectedVariant(v);

    const maxStock = Number(v?.stock ?? 0);
    setVariantQuantity((prev) => {
      if (maxStock <= 0) return 0;
      const next = Math.min(Math.max(1, prev || 1), maxStock);
      // debounce validate
      setQtyError(null);
  ...
  const handleConfirmVariant = async () => {
    ...
    try {
      if (pendingAction === "buy") {
        ...
        // Xử lý image source cho đúng format
        let imageSource;
        if (typeof selectedProduct.image === 'string') {
          imageSource = { uri: selectedProduct.image };
        } else if (selectedProduct.image?.uri) {
          imageSource = { uri: selectedProduct.image.uri };
        } else {
          imageSource = require("../assets/images/pc1.png");
        }
        ...
        return router.push({
          pathname: '/pay',
          params: { selectedProducts: JSON.stringify(payload) }
        });
      }
      ...
      Toast.show({
        type: "error",
        text1: pendingAction === "buy" ? "Mua ngay thất bại!" : "Thêm giỏ hàng thất bại!",
  ...
  const QuickActions = () => (
    ...
    >
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          setInput("VGA gaming tốt nhất cho 1440p trong tầm giá 15 triệu")
        }
      >
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          setInput("Nên chọn RAM DDR4 hay DDR5 cho PC gaming 2024?")
        }
      >
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          setInput("PC tôi bị lag và tự tắt máy, nguyên nhân có thể là gì?")
        }
      >
          ...
          <View style={styles.headerLeft}>
            ...
            <View>
              ...
              <Text style={styles.headerSubtitle}>
                {loading
                  ? "Đang phân tích..."
                  : "PC Specialist • Powered by Gemini"}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <View style={styles.modalHeader}>
                ...
                <TouchableOpacity onPress={() => setShowVariantDialog(false)}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={styles.variantPrice}>
                        ...
                        >
                          {priceShown.toLocaleString("vi-VN")}đ
                        </Text>
                      </View>
                    </TouchableOpacity>
                  );
              ...
              <View style={styles.quantitySection}>
                ...
                <View style={styles.quantityControls}>
                  <TouchableOpacity
  style={[styles.quantityButton, variantQuantity <= 1 && styles.quantityButtonDisabled]}
  onPress={() => {
    // clear lỗi cũ
    setQtyError(null);

    // cập nhật số lượng + debounce validate với server
    setVariantQuantity((q) => {
      const next = Math.max(1, (q || 1) - 1);
      if (qtyDebounceRef.current) clearTimeout(qtyDebounceRef.current);
      qtyDebounceRef.current = setTimeout(() => apiValidateQuantity(next), 300);
      return next;
    });
...
>
  <Ionicons name="remove" size={20} color={variantQuantity <= 1 ? "#ccc" : "#666"} />
</TouchableOpacity>


                  <TextInput
                    value={String(variantQuantity)}
                    onChangeText={(t) => {
                      const num =
                        parseInt((t || "").replace(/[^\d]/g, ""), 10) || 1;
                      const limit = Number(selectedVariant?.stock ?? 0);
                      const next = Math.min(
                        Math.max(1, num),
                        limit > 0 ? limit : 0
                      );
                      setVariantQuantity(next);
                      setQtyError(null);
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setVariantQuantity((q) => {
                        const limit = Number(selectedVariant?.stock ?? 99);
                        const next = Math.min(limit, (q || 1) + 1);
                        setQtyError(null);
                        if (qtyDebounceRef.current)
                          clearTimeout(qtyDebounceRef.current);
                        qtyDebounceRef.current = setTimeout(
                    ...
                    disabled={
                      Number(selectedVariant?.stock ?? 0) <= 0 ||
                      variantQuantity >= Number(selectedVariant?.stock ?? 0)
                    }
                  >
                    <Ionicons
                      name="add"
                ...
                {qtyError && <Text style={{ color:'#e11d48', marginTop:4 }}>{qtyError}</Text>}
              </View>
              ...
              >
                ...
                >
                  <Text style={styles.confirmButtonText}>
                    {pendingAction === "buy" ? "Mua ngay" : "Thêm vào giỏ hàng"}
                  </Text>
                </LinearGradient>
              </TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
  ...
  productInfo: { flex: 1 },
  modalProductName: { fontSize: 14, fontWeight: "600", color: "#111827" },
  modalProductPrice: {
    marginTop: 4,
    fontSize: 16,
    fontWeight: "700",
  ...
  stockInfo: { marginTop: 2, fontSize: 12, color: "#6b7280" },
  variantList: { maxHeight: height * 0.35, marginBottom: 8 },
  variantItem: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
  ...
  selectedVariant: { borderColor: "#667eea", backgroundColor: "#f5f7ff" },
  disabledVariant: { opacity: 0.5 },
  variantInfo: { flex: 1, paddingRight: 8 },
  variantLabel: { fontSize: 14, color: "#111827", fontWeight: "600" },
  variantStock: { fontSize: 12, color: "#6b7280", marginTop: 2 },
  variantPrice: { alignItems: "flex-end" },
  priceText: { fontSize: 14, fontWeight: "700", color: "#e11d48" },
  selectedVariantText: { color: "#374151" },
  quantitySection: { marginTop: 6, marginBottom: 12 },
  quantityLabel: {
    fontSize: 14,
    fontWeight: "600",
    color: "#1f2937",
  ...
  confirmButton: { borderRadius: 12, overflow: "hidden", marginTop: 8 },
