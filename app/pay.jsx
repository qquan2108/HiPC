...
export default function PayScreen() {
  ...
  useEffect(() => {
    ...
    setWards([]); setSelectedDistrict(null); setSelectedWard(null);
  }, [selectedProvince]);
  ...
  useEffect(() => {
    if (selectedProvince && selectedDistrict && selectedWard) {
      ...
      setShippingServices([]); setSelectedService(null); setShippingFee(0);
    }
  ...
  try {
    ...
    setProducts([]);
    Toast.show({ type: 'success', text1: 'Đặt hàng thành công!', text2: 'Cảm ơn bạn.' });
    ...
    console.error(err);
    Toast.show({ type: 'error', text1: 'Đặt hàng thất bại!', text2: err.message || 'Vui lòng thử lại.' });
    setPayStatus('fail');
  }
};
  ...
  return (
    <View style={styles.container}>
      ...
      <PayVoucherModal visible={showVoucher} vouchers={voucherList} selectedVoucher={selectedVoucher} setSelectedVoucher={setSelectedVoucher} setShowVoucher={setShowVoucher} />
      <PayCardModal visible={showCardModal} cards={[]} selectedCard={selectedCard} setSelectedCard={setSelectedCard} setShowCardModal={setShowCardModal} />
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View style={styles.infoBox}>
          <View style={{ flex: 1 }}>
            ...
            <Picker selectedValue={selectedProvince} onValueChange={setSelectedProvince} style={{ marginBottom: 4 }}>
              <Picker.Item label="Chọn tỉnh/thành" value={null} />
              {provinces.map(p => <Picker.Item key={p.ProvinceID} label={p.ProvinceName} value={String(p.ProvinceID)} />)}
            </Picker>
            <Picker selectedValue={selectedDistrict} onValueChange={setSelectedDistrict} enabled={!!selectedProvince} style={{ marginBottom: 4 }}>
              <Picker.Item label="Chọn quận/huyện" value={null} />
              {districts.map(d => <Picker.Item key={d.DistrictID} label={d.DistrictName} value={String(d.DistrictID)} />)}
            </Picker>
            <Picker selectedValue={selectedWard} onValueChange={setSelectedWard} enabled={!!selectedDistrict}>
        ...
        <View style={styles.sectionRow}>
          ...
          <View style={styles.countCircle}><Text style={styles.countText}>{products.length}</Text></View>
        ...
        {/* Shipping */}
        <View style={styles.sectionRow}><Text style={styles.sectionTitle}>Giao hàng</Text></View>  
        {shippingServices.length > 0 ? (
          shippingServices.map(svc => (
            <TouchableOpacity key={svc.service_id} style={[styles.shippingRow, selectedService === svc.service_id && styles.shippingRowActive]} onPress={() => { setSelectedService(svc.service_id); calculateShippingFee(svc.service_id); }}>
              <View style={styles.radioCircle}>{selectedService === svc.service_id && <View style={styles.radioDot}/>}</View>
              <Text style={styles.shippingLabel}>{svc.short_name}</Text>
              <Text style={styles.shippingDesc}>{svc.description}</Text>
              <Text style={styles.shippingPrice}>{shippingFee>0 && selectedService===svc.service_id ? formatCurrency(shippingFee) : ''}</Text>
            </TouchableOpacity>
          ))
        ) : (
        ...
        {/* Payment */}
        <View style={styles.sectionRow}><Text style={styles.sectionTitle}>Thanh toán</Text></View>
        {paymentMethods.map(m => (
          <TouchableOpacity key={m.key} style={[styles.paymentRow, selectedPayment === m.key && styles.paymentRowActive]} onPress={() => setSelectedPayment(m.key)}>
            <View style={styles.radioCircle}>{selectedPayment === m.key && <View style={styles.radioDot}/>}</View>
            <Text style={styles.paymentLabel}>{m.label}</Text>
            {m.key === "bank" && selectedPayment === "bank" && (
              <TouchableOpacity onPress={() => setShowCardModal(true)}>
                <Text style={{ color: "#2979ff", marginLeft: 10, fontWeight: "bold" }}>Chọn thẻ</Text>
              </TouchableOpacity>
            )}
          </TouchableOpacity>
        ))}
        ...
        {/* Totals */}
        <View style={styles.sectionRow}><Text style={styles.sectionTitle}>Tổng cộng</Text></View>
        <View style={{ marginHorizontal: 18, marginBottom: 18 }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
            <Text style={styles.totalLabel}>Tạm tính</Text><Text style={styles.totalValue}>{formatCurrency(subtotal)}</Text>
          </View>
          <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
            <Text style={styles.totalLabel}>Phí giao hàng</Text><Text style={styles.totalValue}>{shippingFee>0 ? formatCurrency(shippingFee) : '—'}</Text>
          </View>
          {discount>0 && (
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
              <Text style={styles.totalLabel}>Giảm giá</Text><Text style={[styles.totalValue,{color:'#2ecc71'}]}>-{formatCurrency(discount)}</Text>
            </View>
          )}
          <View style={{ height: 1, backgroundColor: "#eee", marginVertical: 8 }}/>
          <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
            <Text style={[styles.totalLabel,{fontSize:17}]}>Thành tiền</Text><Text style={[styles.totalValue,{fontSize:20,color:'#2979ff'}]}>{formatCurrency(total)}</Text>
          </View>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: "row", alignItems: "center", paddingHorizontal: 18, paddingTop: 18, paddingBottom: 10, backgroundColor: "#fff" },
  backBtn: { marginRight: 10, padding: 4, borderRadius: 20, backgroundColor: "#eaf3ff", alignItems: "center", justifyContent: "center" },
  headerTitle: { fontWeight: "400", fontSize: 22, color: "#222", marginLeft: 4 },
  infoBox: { flexDirection: "row", backgroundColor: "#fff", borderRadius: 14, margin: 14, padding: 14, elevation: 1 },
  ...
  shippingRow: { flexDirection: "row", alignItems: "center", backgroundColor: "#f5f6fa", borderRadius: 10, margin: 18, padding: 12 },
  shippingRowActive: { backgroundColor: "#eaf3ff", borderWidth: 1.5, borderColor: "#2979ff" },
  radioCircle: { width: 22, height: 22, borderRadius: 11, borderWidth: 2, borderColor: "#2979ff", alignItems: "center", justifyContent: "center", marginRight: 10 },
  radioDot: { width: 12, height: 12, borderRadius: 6, backgroundColor: "#2979ff" },
  shippingLabel: { fontWeight: "bold", marginRight: 8 },
  shippingDesc: { fontSize: 13, color: "#2979ff", backgroundColor: "#eaf3ff", borderRadius: 8, padding: 4, marginRight: 8 },
  shippingPrice: { fontWeight: "bold", color: "#888" },
  paymentRow: { flexDirection: "row", alignItems: "center", backgroundColor: "#f5f6fa", borderRadius: 10, margin: 18, padding: 12 },
  paymentRowActive: { backgroundColor: "#eaf3ff", borderWidth: 1.5, borderColor: "#2979ff" },
  paymentLabel: { fontWeight: "bold", marginLeft: 4 },
  selectedCardBox: { flexDirection: "row", alignItems: "center", backgroundColor: "#fff", borderRadius: 12, marginHorizontal: 18, padding: 12, borderWidth: 1, borderColor: "#eaf3ff" },
  totalLabel: { fontSize: 15, fontWeight: "bold" },
  totalValue: { fontSize: 18, color: "#2979ff", fontWeight: "bold" },
  bottomBar: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", padding: 18, backgroundColor: "#fff", borderTopWidth: 1, borderTopColor: "#eee", position: "absolute", bottom: 0, left: 0, right: 0 },
